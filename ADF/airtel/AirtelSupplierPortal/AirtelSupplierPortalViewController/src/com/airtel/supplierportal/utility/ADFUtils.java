/*****************************************************************************************************
 ** Program Name            - ADFUtils.java
 ** Program Description     - A series of convenience functions for dealing with ADF Bindings.
...
public class ADFUtils implements Serializable {
    ...
    private static final ADFLogger _logger = ADFLogger.createADFLogger(ADFUtils.class);
    private static final long serialVersionUID = 3214467652179579528L;
    ...
    public static void printRow(Row row) {
        _logger.info("START " + row.getKey() +
                           " *********************");
        for (int i = 0; i < row.getAttributeCount(); i++) {
            _logger.info("row[" + row.getAttributeNames()[i] + "]:[" +
                               row.getAttribute(i) + "]");

        }
        _logger.info("END   " + row.getKey() +
                           "*********************\n");
    }
    ...
    /**
     * Generates pretty url for a given node in nav model
     * @param nodeId
     * @return
     * @author Gopal Panda
     */
    public static String getPrettyUrl(String nodeId){
        String expr = "#{navigationContext.defaultNavigationModel.node['"+nodeId+"'].goLinkPrettyUrl}";
        return "/JioApp"+(String)evaluateEL(expr);
    }
    
    /**
     * Redirects to the path provided
     * @param path
     * @author Gopal Panda
     * @throws IOException
     */
    ...
    /**
     * helper method to generate the current request URL path 
     * @param path
     * @author Ravi Kiran
     * @throws IOException
     */
    ...
    /**
     * helper method to set request param
     * 
     * @author Ravi Kiran
     * 
     */
    public static Map getRequestScopeMap() {
